...
def init_images(global_state):
    ...
    state['renderer'].blend_interval = state['params']['blend_interval']
    state['renderer'].reproject_steps = state['params']['reproject_steps']
...
with gr.Blocks() as app:
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=4, min_width=10):
                        ...
                        with gr.Row():
                            form_blend_interval_number = gr.Number(
                                value=50,
                                label="Blend Interval (N)",
                                interactive=True,
                                minimum=1,
                                maximum=200,
                                step=1
                            )
                            form_reproject_steps_number = gr.Number(
                                value=25,
                                label="Reproject Steps (M)",
                                interactive=True,
                                minimum=0,
                                maximum=200,
                                step=1
    ...
    gr.Markdown("""
        ...
        * Note that changing the latent space will reset the image, points and
        ...
        3. Adjust `Blend Interval (N)` and `Reproject Steps (M)` parameters:
        * `Blend Interval (N)`: Interval for feature blending (steps between blending).
        * `Reproject Steps (M)`: Number of re-projection steps to perform.
        * These parameters control the frequency and intensity of feature re-projection during optimization.
        4. Click `Edit Flexible Area` to create a mask and constrain the
    ...
    def on_change_blend_interval(blend_interval, global_state):
        """Update blend interval parameter and re-initialize renderer"""
        global_state['params']['blend_interval'] = int(blend_interval)
        if hasattr(global_state['renderer'], 'init_network'):  # Only update if renderer is initialized
            global_state['renderer'].blend_interval = int(blend_interval)
        return global_state

    form_blend_interval_number.change(
        on_change_blend_interval,
        inputs=[form_blend_interval_number, global_state],
        outputs=[global_state],
    )

    def on_change_reproject_steps(reproject_steps, global_state):
        """Update reproject steps parameter and re-initialize renderer"""
        global_state['params']['reproject_steps'] = int(reproject_steps)
        if hasattr(global_state['renderer'], 'init_network'):  # Only update if renderer is initialized
            global_state['renderer'].reproject_steps = int(reproject_steps)
        return global_state

    form_reproject_steps_number.change(
        on_change_reproject_steps,
        inputs=[form_reproject_steps_number, global_state],
        outputs=[global_state],
    )
